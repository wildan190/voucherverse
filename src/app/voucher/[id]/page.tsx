...
// Removed import VoucherClientPage
...
export default function VoucherPage() {
  ...
  const fetchVoucherDetail = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        // Update document title once voucher data is fetched
        if (fetchedVoucher.name) {
          // Title will be composed by layout's template: `${fetchedVoucher.name} - Latsubnet`
          document.title = fetchedVoucher.name;
        } else {
          document.title = `Voucher ${id}`;
        }
      } else {
      ...
      document.title = `Voucher ${id}`; // Fallback title
    } finally {
      setIsLoading(false);
    }
  }, [id, toast]);
  ...
  // Effect for initial title before data loads
  useEffect(() => {
    if (id && !voucher) {
      document.title = `Voucher ${id}`;
    }
  ...
  const handlePurchaseSuccessOnDetailPage = () => {
    fetchVoucherDetail(); // Re-fetch to update status (e.g., isSold)
    toast({
